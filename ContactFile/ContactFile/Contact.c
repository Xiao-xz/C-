...
void SaveCantact(const Contact* pos)
{
	int i = 0;
	FILE* pfWrite = NULL;
	pfWrite = fopen("Contact.data", "wb");
	if (pfWrite == NULL)
	{
		printf("SaveCantact:%s",strerror(errno));
		return;
	}
	for (i = 0; i < pos->size; i++)
	{
		fwrite(&(pos->data[i]), sizeof(Peoinfo), 1, pfWrite);
	}
	printf("保存成功\n");
	fclose(pfWrite);
	pfWrite = NULL;
